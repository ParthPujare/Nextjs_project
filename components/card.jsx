...
export default function Card({ title, description, id, imgsrc }) {
  return (
    <div className="flex flex-col justify-between border border-gray-300 bg-gray-100 rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300 p-4 w-full max-w-xs sm:max-w-sm md:max-w-xs">
      ...
      <Link href={`/projects/${id}`} className="flex justify-center sm:justify-end pt-4 text-gray-800 font-semibold hover:underline">Learn More </Link>
    </div>
  );
}
